...
const[isHover, setHover]=useState(false)
  ...
  return (
    <div className="bg-[#00a79b] h-full w-full inline-block pb-20 min-h-screen overflow-hidden">
      <motion.div className="px-1, py-1 flex justify-between flex-col md:flex-row items-center">
        ...
        <motion.div initial={{y:"-20%"}} animate={{y:0}} transition={{duration:1}} className="flex" >
          <p className=" sm:text-base md:text-lg notranslate font-family:Fredoka text-xl rounded-bl-2xl rounded-br-2xl px-10 mb-6 py-3 bg-white inline">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center px-4 lg:px-20 mt-10 gap-10">
        <motion.div
          initial={{
            x: -100,
            opacity: 0,
        ...
        >
          <div className="flex py-30 sm:flex-row items-center">
            <div>
              <img src={logo} alt="logo" className="w-45 h-45" />
            </div>
            <div>
              <p className="text-7xl sm:text-5xl lg:text-7xl text-white font-['Fredoka_One'] py-10 px-5">
                pharmacy
              </p>
            </div>
          </div>
          <div className="flex justify-between bg-white h-14 w-150 relative bottom-25 left-25 px-5 py-1 rounded-xl lg:justify-start max-w-full">
            <img
              src={Heart}
              alt="heart"
            ...
            <p className="text-3xl font-['Fredoka_One'] mb-2 sm:text-lg md:text-2xl">
        ...
        <motion.div
          ...
          className=" relative max-w-xl px-20 py-15"
        >
          <motion.img
            src={img6}
            alt="icon"
            className="absolute  md:block z-0 h-30 top-5 right-100"
            animate={
          ...
          <motion.img
            ...
            animate={isHover ? { x: 5, y: -5 } : { x: 0, y: 0 }}
            transition={{ type: "spring", stiffness: 100 }}
            className="absolute z-1 h-20 top-110 left-8"
          />
          <motion.img
            src={img5}
            alt="map"
            animate={isHover ? { x: 5, y: -5 } : { x: 0, y: 0 }}
            transition={{ type: "spring", stiffness: 100 }}
            className="absolute z-1 h-30 top-120 right-8"
          />
          <motion.img
            src={rectangle1}
            alt="mainimg"
