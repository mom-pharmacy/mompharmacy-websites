...
const DownloadApp = ({setDownloadApp,downloadapp}) => {
  return (
    <AnimatePresence mode="wait">
     { downloadapp &&<motion.div initial={{x:1500,display:"none"}} animate={{x:0,display:"flex"}} transition={{duration:0.2}} exit={{x:1500}} className="flex fixed h-screen w-screen justify-center items-center">     
    <div className="flex outline-1 w-200 items-center justify-center gap-10 p-3 fixed bg-white rounded-2xl outline-[#00A79B]  ">
      ...
      <div>
          <button className='text-[#00A79B] cursor-pointer w-10 h-10 relative -top-53 -right-7 bg-white rounded-full'  onClick={()=>setDownloadApp(false)}>
