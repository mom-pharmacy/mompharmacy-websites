...
const EarlyAccessForm = ({ earlyaccess, setEarlyAccess }) => {
  ...
  const { postEmailAccess } = useEarlyAccess()
  const handleClick = () => {
    postEmailAccess({email})
    setEmail("")
  }
  return (
          ...
          <div className='xl:w-140 xl:h-70 w-90 h-70 md:w-140 md:h-70 bg-white shadow-2 shadow-xl flex flex-col  justify-center gap-2 px-5 py-2 xl:m-10 md:m-10  rounded-4xl   border-8  border-[#00A79B] '>
            ...
            <div className='flex justify-between gap-5'>
              <input type='email' name="email" value={email} onChange={(e) => setEmail(e.target.value)}
                placeholder='enter your email' className=' outline-1 outline-[#00A79B] rounded-sm px-2 py-3 w-100' />
              <button type="submit" onClick={handleClick} className=' outline-1 outline-[#00A79B]  bg-[#00A79B] text-white rounded-sm  hover:bg-[#FADB14]  hover:text-black px-4 py-2'>Submit</button>
            </div>
          </div>
